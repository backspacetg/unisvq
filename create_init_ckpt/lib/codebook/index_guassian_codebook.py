...
class LinearGuassian_codebook(nn.Module):
    ...
    def __init__(self, inference=False, **kwargs):
        ...
        proj_mean = (2**_LI_CODEBIT-1)/2
        proj_scale = (torch.pow(torch.arange(2**_LI_CODEBIT, dtype=torch.bfloat16), 2).mean().item() - proj_mean**2)
        # glog.info("{} {}".format(proj_mean, proj_scale))
    ...
    def quantize(self, w, return_idx=True, **kwargs):
        ...
        # glog.debug(f'before quantize: {w.shape}, {w.mean().item()}, {w.std().item()}, {torch.norm(w, dim=1).tolist()}')
...
class LinearGuassianLinear(nn.Module):
    ...
    def __init__(self, device, **kwarg):
        super().__init__()
        self.codebook = LinearGuassian_codebook().to(torch.bfloat16).to(device)
        self.skip_mix_hadk = kwarg.get('skip_mix_hadk', False)
        self.scale = 32
    ...
    def forward(self, input, Qidxs_list, SU, SV, had_left_T, had_right, K_left, K_right, rescale_WH=False, scaleWH=None, train_mode=False, **kwargs):
        ...
        x = matmul_hadU_cuda(x, had_left_T, K_left, skip_mix=self.skip_mix_hadk) / self.scale
    
        W_decompressed = self.codebook.by_idxs(Qidxs_list[0])
        x = (x.to(torch.bfloat16) @ W_decompressed.T).to(torch.float32)

        x = matmul_hadU_cuda(x, had_right, K_right, skip_mix=self.skip_mix_hadk)
        x = x * SV * self.scale
