...
class LinearIndex_codebook(nn.Module):
    ...
    def __init__(self, inference=False, **kwargs):
        ...
        mean = -2**(_LI_CODEBIT-1)/_LI_CODESZ
        scale = (torch.pow(torch.arange(-2**(_LI_CODEBIT-1), 2**(_LI_CODEBIT-1), dtype=torch.float), 2).mean().item() - mean**2)
        # glog.info(f"scale: {scale}, mean: {mean}")
        input_index_sample = 1.0/math.sqrt(scale)*(input_index_sample-2**(_LI_CODEBIT-1))
        fake_codebook = linear_proj(input_index_sample) 
        self.register_buffer('linear_proj', linear_proj.weight.data.clone())

        fake_codebook = 1/math.sqrt(scale)*(input_index_sample-mean)
...
class LinearIndexLinear(nn.Module):
    
    def __init__(self, device):
        super().__init__()
        self.codebook = LinearIndex_codebook().to(torch.float16).to(device)
        self.scale = 32
