...
class E8P12_codebook(nn.Module):
    ...
    def quantize(self, X, return_idx=True, **kwargs):
        ...
        # glog.info(f'before quantize: {X.shape}, {X.mean().item()}, {X.std().item()}')
        # glog.info(f'after quantize: {final_vals.shape}, {final_vals.mean().item()}, {final_vals.std().item()}')
        # glog.info(f'cb: {self.grid_part.shape}, {self.grid_part.mean(), self.grid_part.std()}')
...
class QuantizedE8P12Linear(nn.Module):
    ...
    def cache_WH(self, n, m, Qidxs_list, had_left, had_right, K_left, K_right, **kwargs):
        # 解包之后直接就是转置的
