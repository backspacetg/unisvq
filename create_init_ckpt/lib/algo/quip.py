...
def RHT_H(H, SU, skip_mix=False):
    return utils.matmul_hadUt(utils.matmul_hadUt(H * SU, skip_mix=skip_mix).T * SU, skip_mix=skip_mix)


def RHT_W(W, SU, SV, skip_mix=False):
    return utils.matmul_hadUt(utils.matmul_hadUt(W.T * SV, skip_mix=skip_mix).T * SU, skip_mix=skip_mix)
...
def incoherence_preprocess(H, W, args):
    ...
    if args.incoh_mode == "had":
        ...
        Hr = RHT_H(Hr, SU, skip_mix=args.skip_mix_hadk)
        Wr = RHT_W(Wr, SU, SV, skip_mix=args.skip_mix_hadk)
    ...
    while retry <= 3:
        try:
            if retry == 0:
                ...
                glog.warning(f"Hr is not positive-definite; scaling sigma reg: {args.sigma_reg} -> {(10**retry)*args.sigma_reg}")
                Lhr = torch.linalg.cholesky(Hr + (10**retry)*args.sigma_reg*torch.eye(Hr.size(0), dtype=Hr.dtype, device=Hr.device))
            break
        except:
            retry += 1
...
def incoherence_process(hatWr, SU, SV, scaleWH, args):
    ...
    if args.incoh_mode == 'had':
        hatWr = (utils.matmul_hadU(
            (utils.matmul_hadU(hatWr, skip_mix=args.skip_mix_hadk) * SU.to(device)).T, skip_mix=args.skip_mix_hadk) * SV.to(device)).T
...
def LDLQ(Wr, Hr, L, D, cb, args):
    ...
    for _ in range(args.quip_tune_iters):
        for k in reversed(range(n // cb.codesz)):
            WXWX = hatWr[:, (cb.codesz * k):(cb.codesz * (k + 1))] + (Wr - hatWr) @ \
                Hr[:, (cb.codesz * k):(cb.codesz * (k + 1))] @ \
                torch.linalg.inv(Hr[(cb.codesz * k):(cb.codesz * (k + 1)), (cb.codesz * k):(cb.codesz * (k + 1))])
    ...
    for ie in range(args.quip_tune_iters):
        ...
        for cur_col in range(n // cb.codesz, 0, -buf_size):
            ...
            for i in reversed(range(buf_size)):
                ...
                b_delta_T[cb.codesz * i:cb.codesz * (i + 1)] += b_hatWr_T[cb.codesz * i:cb.codesz * (i + 1)]
...
def quantize(H_orig, W_orig, rank, codebook_orig, args, device='cpu'):
    ...
    if incoh_out is None:
        ...
        glog.debug('incoherence_preprocess failed, recomputing in fp64')
        del H, W, codebook
        utils.clean()
    ...
    if block_LDL_out is None:
        ...
        new_args = copy.deepcopy(args)
        new_args.use_fp64 = True
        glog.debug('block_LDL failed, recomputing in fp64')
    ...
    # LDLQ
    Wscale = Wr.square().mean().sqrt() / getattr(codebook, "codebook_scale", 1.0)
    ...
    H_orig = H_orig.to(device=device, dtype=W_orig.dtype)
    for wscale_grid_search_factor in [1.0]: # torch.linspace(0.8, 1.2, args.scale_search_iters):
        Wscale_search = Wscale*wscale_grid_search_factor

        Wr_search = Wr / Wscale_search
        codebook = codebook.to(device)
        if args.no_use_buffered:
            hatWr_search, Qidx_search = LDLQ(Wr_search, Hr, L, D, codebook, args)
        elif args.lowmem_ldlq or args.use_fp64:
            hatWr_search, Qidx_search = LDLQ_buffered_lowmem(Wr_search, Hr, L, D, codebook,args, buf_cols=128)
        else:
            hatWr_search, Qidx_search = LDLQ_buffered(Wr_search, Hr, L, D, codebook, args, buf_cols=128)
        
        hatWr_search = hatWr_search * Wscale_search
        hatW_search = incoherence_process(hatWr_search, SU, SV, scaleWH, args).to(W_orig.dtype)
        err_proxy = (((hatW_search - W_orig) @ H_orig) * (hatW_search - W_orig)).sum() / ((W_orig @ H_orig) * W_orig).sum().item()
        if err_proxy < best_error:
            best_Wscale = Wscale_search
            best_hatWr = hatWr_search
            best_Qidxs = Qidx_search
            best_error = err_proxy
        # glog.info(f"err {err_proxy:.4f} at scale {wscale_grid_search_factor:.4f}")
        glog.info("before quantize: W_mean={:.4f}, W_std={:.4f}".format(Wr.mean().item(), Wr.std().item()))
        glog.info("after quantize: W_mean={:.4f}, W_std={:.4f}".format(best_hatWr.mean().item(), best_hatWr.std().item()))
    # glog.info(f"best err {best_error:.4f} at scale {best_Wscale:.4f}")
    ...
    # best_hatWr = best_hatWr * best_Wscale

    # low rank correction
    if args.lora_rank > 0:
        best_hatWr, A, B = low_rank_process(Wo, best_hatWr, Lhr, args)
        A = A.half().cpu()
        B = B.half().cpu()
    else:
        A, B = None, None
    ...
    hatW = incoherence_process(best_hatWr, SU, SV, scaleWH, args)

    Qidxs = codebook.maybe_pack_idxs(best_Qidxs)

    attr = {
        'Qidxs': Qidxs.to(orig_device),
        'A': A,
        'B': B,
        'SU': SU.to(torch.bfloat16).to(orig_device),
        'SV': (SV * best_Wscale.to(SV.device)).to(torch.bfloat16).to(orig_device),  # fuse Wscale into SV
        'scaleWH': scaleWH,
    }
...
def quantize_linear(weights, save_path, hessian_path, cb, args, device='cpu', bias=None, save_result=True):
    dtype_ = torch.float64 if args.use_fp64 else torch.float32
    ...
    if os.path.exists(save_path):
        glog.info(f"{save_path} already exists, skip it")
        return

    H_data = torch.load(hessian_path, map_location=torch.device('cpu'), weights_only=True)
    H = utils.flat_to_sym(H_data['flatH'], H_data['n'])
    mu = H_data['mu']
    H.add_(mu[None, :] * mu[:, None])
    n = H_data['n']
    W = torch.vstack([weights[i].to(dtype_) / scales[i] for i in range(len(weights))]).to(dtype_)
    H = utils.regularize_H(H, n, args.sigma_reg)
    hatW, attr = quantize(H, W, args.lora_rank, cb, args, device)
    # print("after quant", attr["Qidxs"].max())
    if len(scales) == 1:
        attr['SV'] *= scales[0]
        scales = [1.0]
    attr.update({
...
def quantize_linear_with_hessian(weights, save_path, H_data, cb, args, device='cpu', bias=None, save_result=True):
    dtype_ = torch.float64 if args.use_fp64 else torch.float32
    ...
    H = H_data['H'] # utils.flat_to_sym(H_data['flatH'], H_data['n'])
    mu = H_data['mu']
    H.add_(mu[None, :] * mu[:, None])
    n = H_data['n']
    W = torch.vstack([weights[i].to(dtype_) / scales[i] for i in range(len(weights))]).to(dtype=dtype_, device=device)
    H = utils.regularize_H(H, n, args.sigma_reg).to(device=device)
    hatW, attr = quantize(H, W, args.lora_rank, cb, args, device)
