...
class QuantizedLinear(nn.Module):
    ...
    def __init__(
        ...
        self.has_bias = bias
        if self.has_bias:
            self.register_buffer('bias', torch.ones(out_features, dtype=torch.bfloat16))
        ...
        if pack_out:
            self.register_buffer("Qidxs", torch.zeros(int(out_features / packsz), int(in_features / codesz), dtype=dtype_from_str(idx_dtype)))
        else:
            self.register_buffer("Qidxs", torch.zeros(out_features, int(in_features / (codesz * packsz)), dtype=dtype_from_str(idx_dtype)))

        self.register_buffer("codebook_id", torch.tensor(codebook_id, dtype=torch.int))
        self.register_buffer("SU", torch.ones(in_features, dtype=torch.bfloat16))
        self.register_buffer("SV", torch.ones(out_features, dtype=torch.bfloat16))
    ...
    def ckpt_forward(self, input):
        return torch.utils.checkpoint.checkpoint(self.no_ckpt_forward, input, use_reentrant=True)
