...
def sample_rp1t(tokenizer, size=128, ctx_size=2048, nproc=1):
    dataset = load_dataset('/home/wanghaoyu2/data/RedPajama-Data-1T-Sample',split='train')
...
def sample_rp1t_concat(tokenizer, size=128, ctx_size=2048, nproc=1):
    ...
    concat = []
    total_len = 0
    pbar = tqdm.tqdm(total=ctx_size*size, desc="tokening data")
    while total_len < ctx_size * size:
        seqs = dataset[torch.randint(len(dataset), (128, ))]['text']
        tokens = tokenizer(
            seqs,
            return_tensors='pt',
            truncation=True,
        ...
        lens = attention_masks.sum(dim=-1)
        for j in range(len(input_ids)):
            concat += [input_ids[j][:lens[j]]]
            total_len += lens[j]
            pbar.update(lens[j].squeeze().item())
    concat = torch.cat(concat)[:ctx_size * size]
    return concat.reshape(size, ctx_size).contiguous()


def sample_jsonl_concat(dataset_path, tokenizer, size=128, ctx_size=2048, nproc=1):
...
def sample_falcon_refinedweb(tokenizer, size=128, ctx_size=2048, nproc=1):
    dataset = load_dataset('tiiuae/falcon-refinedweb', streaming=True, split='train')
...
def load_quip(save_name, cb, args, device, return_scales=False):
    ...
    if args.incoh_mode == "had":
        hatW = (matmul_hadU((matmul_hadU(hatWr, mix=args.skip_mix_hadk) * SU).T, mix=args.skip_mix_hadk) * SV).T
...
def unpack_quip(module, saved_layer, codebook_id, codesz, load_tuneable=False):
    (m, n) = saved_layer['Qidxs'].shape
    if codebook_id in codebook.cache_permute_set:
        module.Qidxs.copy_(saved_layer['Qidxs'].view(m, n // codesz, codesz).permute(1, 0, 2).reshape(m, n).contiguous())
    else:
    ...
    if saved_layer['bias'] is not None:
        module.bias.copy_(saved_layer['bias'].to(module.bias.dtype))
    
    if load_tuneable:
        module.codebook_class.codebook.linear_proj.weight.copy_(
            saved_layer['codebook_class.codebook.linear_proj.weight'].to(module.codebook_class.codebook.linear_proj.weight.dtype)
        )
        module.codebook_class.codebook.linear_proj.bias.copy_(
            saved_layer['codebook_class.codebook.linear_proj.bias'].to(module.codebook_class.codebook.linear_proj.bias.dtype)
        )
...
def split_data(X, Y, args):
    ...
    train_dl = DataLoader(train_ds, batch_size=args.ft_bs, pin_memory=True, shuffle=True)
    valid_dl = DataLoader(valid_ds, batch_size=args.ft_bs, pin_memory=True, shuffle=False)
