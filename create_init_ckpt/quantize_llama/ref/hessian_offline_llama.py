...
def main(args):
    ...
    devset = utils.sample_rp1t(tokenizer, args.devset_size, args.ctx_size, nproc=args.sample_proc)
    ...
    for transformer_layer_index in range(len(model.model.layers)):
        ...
        forward_procs = []
        for i in range(ngpus):
            p = mp.Process(target=forward_layer, args=(transformer_layer, position_ids,attention_mask, args.batch_size, i, in_q, out_q))
            p.start()
