...
def qep_modify_weights(layer, layer_outputs, args):
    ...
    for parent_name, linear_name, group_name in quant_order:
        ...
        # glog.info("[MEAN] original: {}, added: {}".format(linear_module.weight.data.mean(), torch.mm(linear_module.weight.data, H_factor).mean()))
        # glog.info("[MAX] original: {}, added: {}".format(linear_module.weight.data.max(), torch.mm(linear_module.weight.data, H_factor).max()))
        # glog.info("[STD] original: {}, added: {}".format(linear_module.weight.data.std(), torch.mm(linear_module.weight.data, H_factor).std()))
...
def main(args):
    ...
    for i in tqdm.tqdm(range(len(model.model.layers))):
        ...
        # with torch.no_grad():
        #     for j in pbar:
        #         for key in ['input_after_quantize']: 
        #             result = model.model.layers[i](
        #                 embedding_cache[key][args.batch_size*j:args.batch_size*(j + 1)].to(device),
        #                 position_embeddings=position_embeddings,
        #                 attention_mask=attention_mask,
