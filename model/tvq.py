...
from .matmul_had import matmul_hadU_cuda_blockwise
...
class DGEFunction(torch.autograd.Function):
    ...
    def forward(ctx, w, qmax:int=3, k:float=5.0, power_clamp_max: float=3.0):
        ...
        ctx.qmax = qmax
        # ctx.grad_update_rate = grad_update_rate
    ...
    def backward(ctx, grad_output):
        ...
        qmax = ctx.qmax
        # grad_update_rate = ctx.grad_update_rate
...
class CodeBookGenerator(nn.Module):
    ...
    def quant_weight(self, ori_weight) -> torch.Tensor:
        ...
        if type(self.max_var) == nn.Parameter or type(self.max_var) == torch.Tensor:
            ...
            # print(f"rank {rank}, linear {self.linear_name}, using tensor max var {max_var}")
        else:
            max_var = self.max_var
            # print(f"rank {rank}, linear {self.linear_name}, using float max var {max_var}")
